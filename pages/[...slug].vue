    ...
    <template v-if="data.images?.length">
      ...
      <div class="w-full grid grid-cols-4 gap-2">
        <div v-for="img in data.images" :key="img" class="relative w-full pb-[100%]">
          <img :src="absoluteURL(data._path, img)" class="w-full h-full object-cover absolute">
        </div>
      </div>
    </template>
  </div>
...
const dateFormatterEn = new Intl.DateTimeFormat('fa-IR', {
...
const name = computed(() => $lang === 'en' ? data.value.name : data.value.nameFa)
const title = `${name.value} - ${data.value.death?.city} - ${data.value.death?.province}`
const image = computed(() => data.value?.images?.[0] ? absoluteURL(data.value?._path, data.value?.images?.[0]) : '')
